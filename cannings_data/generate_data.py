""" Set of usefull functions to compute the probability and the time to fixation and store them in a json file.
...
def initialise_file(file_name, pop_size, p0, selection_type):
    """ Initialise the hyper parameters (pop_size, p0, selection) in  the json file where the data will be stored."""
...
def check_hyper_parameters(file_name, model, pop_size, p0, selection_type):
    """ Check that the hyper parmeters (pop_size, p0, selection) in argument are the same as the one in file_name.
    ...
    everything_fine = hyper_parameters['model'] == model and hyper_parameters[
        'population_size'] == pop_size and hyper_parameters['p0'] == p0 and hyper_parameters['selection'] == selection_type
    return everything_fine


def initialise_data(file_name, pop_size, alpha, p0, selection_coeff, selection_type, model='cannings'):
    """ Initialise the parameters (alpha, selection_coeff) in the json file where the data will be stored."""
    ...
    if not everything_fine:
        raise Exception(f'The file {file_name} has not been the right hyperameters.')
...
def generate_fixation(file_name, pop_size, alpha, p0, selection_coeff, selection_type='fecundity', model='cannings', n_iter=1):
