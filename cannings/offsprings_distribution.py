""" Tools for the offspring distribution in a Cannings model where
    - the probability that an individidual has no offsprings is p0
        (0 <= p0 <= 1)
...
def generate_offsprings(alpha, p0, nb_individuals=1):
    """ Generate randomly the number of offsprings of nb_individuals individuals
    in the Cannings model with parameters alpha and p0

    >>> generate_offsprings(alpha=1.4, p0=1)
    0
    >>> generate_offsprings(alpha=2, p0=0) == 0
    False
    >>> np.random.seed(2)
    >>> generate_offsprings(alpha=1, p0=0)
    2
    >>> generate_offsprings(alpha=2, p0=0.1, nb_individuals=50)
    66
    """
    ...
    nb_offsprings = (np.exp(1/alpha * np.log((1-p0)/u))).astype(int)

    return nb_offsprings.sum()
